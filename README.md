...
| 使用场景               | 核心技能组合                          | 典型流程                                      | 推荐中文 Prompt 关键词                     | 风险风格参考     |
|-------------------------------|-----------------------------------------------|----------------------------------------------------|-----------------------------------------------------|-------------------------|
| **Meme 币交易**             | OpenNews + GMGN + PancakeSwap/Uniswap + Binance | 新闻热度 → Smart Money 验证 → DEX 上车 + CEX 对冲   | “用 OpenNews 查 Meme 新闻 + GMGN smart money，给出 PancakeSwap 上车建议”     | 激进 / 稳健         |
| **新闻驱动 Meme**         | OpenNews + GMGN + DEX + CEX                   | 高分 Meme 新闻 → 情绪验证 → DEX/CEX 快速操作     | “用 OpenNews 查爆火 Meme 新闻（score > 80），结合 GMGN 给出上车建议” | 激进 / 稳健         |
| **宏观新闻 + 资金费率套利** | OpenNews + 多 CEX (资金费率监控)         | 宏观/利率新闻 → 跨所费率差异 → CEX 套利         | “用 OpenNews 查美联储新闻，检查 Binance/Bybit 资金费率差异套利” | 激进 / 稳健         |
| **OnChain 鲸鱼预警**       | OpenNews + GMGN + CEX                         | 鲸鱼大额交易预警 → GMGN 验证 → CEX 快速响应   | “用 OpenNews 查 Hyperliquid 鲸鱼动向，结合 GMGN 给出 Binance 快速响应建议” | 激进                 |
| **预测市场交易**       | Polymarket + CEX                              | 事件市场分析 → 下注 → CEX 对冲               | “用 Polymarket 查热门事件，分析后在 Binance 对冲”         | 中等                 |
| **新闻 + Polymarket 事件** | OpenNews + Polymarket + CEX                   | 新闻信号 → 事件市场验证 → CEX 对冲           | “用 OpenNews 查重要新闻，结合 Polymarket 事件分析后对冲”     | 稳健                 |
| **链上 DeFi 操作**         | cryptoskills + Uniswap/PancakeSwap            | Swap / Liquidity / Farming                       | “在 PancakeSwap 为 XXX token 添加流动性并开 Farming”       | 低                     |
| **CEX 自动化**               | Binance/OKX/Bybit + cryptoskills              | 现货/永续/期权交易 + 网格/DCA            | “在 Binance 上开 BTCUSDT 网格策略”               | 低                     |

> **提示**：上表为常见场景的最优组合方案。风险风格仅供参考，具体流程和更多 prompt 请参考 **[USAGE.md](./USAGE.md)**。
...
**AI 中文提示词示例**：
> “用 OpenNews 查一下今天 BTC ETF 相关的高分新闻，分析信号后建议 Binance 或 PancakeSwap 操作。”
> “检查最近 Hyperliquid 鲸鱼动向和 KOL 交易，并结合 GMGN 给出跟进建议。”
...
- [OpenNews MCP](https://github.com/6551Team/opennews-mcp) & [ClawHub](https://clawhub.ai/infra403/opennews-mcp)
