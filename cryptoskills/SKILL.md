...
### 示例 1: Aave v3 供应资产（viem）

```ts
// 使用 viem 在 Aave v3 中 supply USDC
const hash = await walletClient.writeContract({
  address: '0x87870Bca3F3fD6335C3afA6d2a3B0e2E8e5e5e5e', // Aave Pool
  abi: aavePoolAbi,
  functionName: 'supply',
  args: [
...
### 示例 2: Jupiter 交换（Solana Agent Kit）

```ts
// 使用 solana-agent-kit 执行 Jupiter swap
...
- **多链适酌**：根据你的目标链（EVM vs Solana vs Aptos 等）给出针对性 SDK 和 gas 策略。
