...
- **binance-agentic-wallet**：支持钱包管理、转账、DEX 交换、限价单、预测市场交易（BSC/ETH/Base/Solana）
...
**优势**：
- 与 Binance Web3 Wallet 深度集成
- 支持多条链（BSC, Ethereum, Base, Solana）
- 安全、可信赖的市场数据和操作能力
...
### 示例 3: Binance Agentic Wallet - Token Swap

```ts
// 使用 binance-agentic-wallet 执行 DEX 交换
...
### 示例 4: 查询钱包持仓 (query-address-info)

```ts
// 查询某个地址的持仓情况
const portfolio = await queryAddressInfo('0x...');
console.log(portfolio.tokens); // 列出所有代币及价格
```
